      ...
      {fsBottomBar ? (
        <div className="fullscreen-bottom-bar">
          ...
          <div className="fs-bb-transport">
            ...
            <button aria-label={state.isPlaying ? "Pause" : "Play"} className="play-button" disabled={!track || !track.available} onClick={() => void playbackStore.toggle()} type="button"><Icon name={state.isPlaying ? "pause" : "play"} size={20} /></button>
            <button aria-label="Next" disabled={!track} onClick={() => void playbackStore.next()} type="button"><Icon name="skip-forward" size={18} /></button>
          </div>
          <div className="fs-bb-progress">
            <time>{formatTime(state.positionMs)}</time>
            <input aria-label="Playback position" disabled={!track || duration <= 0} max={duration || 1} min="0" onChange={(event) => playbackStore.seek(Number(event.target.value))} type="range" value={Math.min(state.positionMs, duration || 1)} />
            <time>{formatTime(duration)}</time>
          </div>
          <div className="fs-bb-volume">
            <button aria-label={state.muted ? "Unmute" : "Mute"} onClick={() => playbackStore.toggleMute()} type="button"><Icon name="volume" size={16} /></button>
            <input aria-label="Volume" max="1" min="0" onChange={(event) => playbackStore.setVolume(Number(event.target.value))} step=".01" type="range" value={state.volume} />
          </div>
        </div>
      ) : null}
    </div>
