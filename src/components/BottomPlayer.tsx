...
export function BottomPlayer({ library }: { library?: NormalizedLibrary }) {
  ...
  return <footer className="bottom-player" aria-label="Playback controls">
    <div className="player-track"><img className="player-art" src={track?.artwork?.src || orbitArt} alt="" /><div><strong>{track?.title || "Nothing playing"}</strong><small>{artist || (track ? "Unknown artist" : "Choose a local track")}</small></div><button aria-label="Favorite" disabled={!track} type="button">♡</button></div>
    ...
    <div className="player-tools"><button aria-label="Queue" type="button"><Icon name="library" size={17} /></button><button aria-label={state.muted ? "Unmute" : "Mute"} onClick={() => playbackStore.toggleMute()} type="button"><Icon name="volume" size={18} /></button><input aria-label="Volume" max="1" min="0" onChange={(event) => playbackStore.setVolume(Number(event.target.value))} step=".01" type="range" value={state.volume} /><button aria-label="Mini player" type="button"><Icon name="panel" size={17} /></button></div>
  </footer>;
}
